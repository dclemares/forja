...
export function FoodForm({ initial, submitLabel = 'Guardar', onSave }: { initial?: Partial<Food>; submitLabel?: string; onSave: (food: FoodInput) => void }) {
  ...
  return (
    <div style={{ padding: '4px 2px 12px' }}>
      <input autoFocus placeholder="Nombre del alimento" value={name} onChange={(e) => setName(e.target.value)} style={inp} />
      <input placeholder="Marca (opcional)" value={brand} onChange={(e) => setBrand(e.target.value)} style={{ ...inp, marginTop: 10 }} />
