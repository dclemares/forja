    ...
    <Sheet open={open} onClose={onClose} title={initial ? 'Editar comida' : 'Nueva comida'}>
      {view === 'edit' ? (
        <div style={{ padding: '4px 2px 12px' }}>
          <input autoFocus={!initial} placeholder="Nombre (p. ej. Batido post-entreno)" value={name} onChange={(e) => setName(e.target.value)} style={inp} />
        ...
        <div style={{ paddingBottom: 8 }}>
          <input autoFocus placeholder="Buscar alimento…" value={q} onChange={(e) => setQ(e.target.value)} style={{ ...inp, marginBottom: 6 }} />
          {addList.map((f) => (
